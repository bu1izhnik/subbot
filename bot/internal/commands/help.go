	...
	_, err := api.Send(tgbotapi.NewMessage(
		...
		fmt.Sprintf(
			"Данный бот позволяет подписывать группы на телеграм каналы: "+
				...
				"(если в группе несколько не один чат, а несколько тем, то бот будет пересылать сообщения из канала в ту тему, где была использована команда)\n"+
				...
				"В данный момент бот не подписывается на приватные каналы и каналы с ограничением на пересылку сообщений\n"+
